(function () {
  ...
  function renderChrome() {
    ...
    if (header) {
      header.innerHTML = `<header class="site-header"><a class="brand" href="index.html" aria-label="kiyo米·Archive 首页"><span class="brand-dot blue"></span><span>${escapeHtml(data.site.title)}</span><span class="brand-dot green"></span></a><button class="menu-toggle" type="button" aria-expanded="false" aria-controls="site-nav"><span></span><span></span><span></span><b class="sr-only">打开导航</b></button><nav id="site-nav" class="site-nav" aria-label="主导航">${navItems.map(([id, label, href]) => `<a href="${href}"${currentPage === id ? ' aria-current="page"' : ""}>${label}</a>`).join("")}</nav></header>`;
  ...
  function renderTimeline() {
    ...
    const draw = (year = "all") => { const items = year === "all" ? sorted : sorted.filter((event) => event.date.startsWith(year)); root.innerHTML = items.length ? items.map((event) => `<article class="timeline-entry reveal" id="${escapeHtml(event.id)}"><time datetime="${escapeHtml(event.date)}">${escapeHtml(event.date.replaceAll("-", "."))}</time><span class="timeline-node" aria-hidden="true"></span><div><h2>${escapeHtml(event.title)}</h2><p>${escapeHtml(event.summary)}</p><a href="${escapeHtml(event.source)}" target="_blank" rel="noopener noreferrer">查看原始来源 <span aria-hidden="true">↗</span></a></div></article>`).join("") : emptyState("时间线正在整理", "真实事件及原始来源整理完成后，将按时间倒序出现在这里。"); observeReveals(); };
