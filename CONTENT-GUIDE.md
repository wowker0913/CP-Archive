...
每条正式事件必须有日期、标题、简述和原始来源。页面默认按日期从新到旧排列。左侧可以按年份查看，右侧可以切换倒序或正序。页面顶部的“一起走过的时光”居中，TIMELINE 在标题下方；标题和引导句在 `timeline.html` 的 `.timeline-intro` 修改。Profile 的“关于她们”和 Archive 的“档案总览”也是同样排法，英文分别在各自标题下方。
...
`id` 现在不负责跳转，也不必写成和日期一样。档案目录默认用 `date` 定位，所以日期不同的事件，即使编号相同，也能各自跳到准确位置。现有事件的编号先保持不动。只有同一天有两条及以上时，才给这几条分别填写不同的短编号；页面会改用编号定位。短编号发布后不要随意更改。
